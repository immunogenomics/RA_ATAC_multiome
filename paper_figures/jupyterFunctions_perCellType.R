...
donor_prop_comp_plot <- function(ID_conv_df,scATAC_meta,CITE_meta,
                                 ...
                                 mLab='Class proportion\nin CITE samples',tSize=8,tVec=NA,clustColors=NA){
    ...
    myplots <- list()
    i=1
    for(cs in unique(comp_scsnATAC_sID_df$scsnATAC)){
...
CNA_umap_plots <- function(toPlot,xCol,yCol,corrCol,thisTitle='',xLab='UMAP1',yLab='UMAP2',colorLab='r',smallPt=FALSE,
    ...
    if(smallPt) {g <- g + geom_point(size=0.001,alpha=0.5)} else {g <- g + geom_point(size=1,alpha=0.5)}
