...
arg_obj = ArgumentParser(description='keep duplicates between cells, not within cells while maintaining read/mate pair status using read/mate start/CIGAR')
arg_obj.add_argument('bam', metavar = 'bam', help='bam file')
arg_obj.add_argument('region', metavar = 'region', help='format: chr:start-stop or BED file')
arg_obj.add_argument('outFile', metavar = 'outFile', help='output file; deduplicated bam')
...
for i,line in enumerate(IN):
    ...
    MC = [p[5:] for p in tabs if len(p)>5 and p[0:4]=='MC:Z'][0]
    key = "-".join([tabs[2],tabs[3],tabs[5],tabs[7],MC,CB])
    ...
    if curr==prev:
        ...
        """Exclude 1 key of read start==mate start, read cigar!=mate cigar pairs"""
        removeKeys = []
        for k in dic.keys():
            k_vals = k.split('-')
            #only want to remove one of the pair, not both, so < not !=
            if k_vals[1]==k_vals[3] and k_vals[2]<k_vals[4]:
                removeKeys.append(k)
        ...
        for k in dic.keys():
            ...
            if not outputPrint_flag and len(chosen_qnames)>0:
                outputPrint_flag = True
...
if not outputPrint_flag:
    getOneShellOutput("rm %s"%(intermediate_outFile))
    raise Exception("No output generated.")

outSam = args.outFile.rsplit('.',1)[0]+'.sam'
...
if args.include_unpaired:
    cmd="mv %s %s"%(intermediate_outFile,outSam)
    ret = getOneShellOutput(cmd)
    if len(ret[1])!=0:
    ...
    cmd = 'awk \'BEGIN{OFS=FS="%s"} FNR==NR{a[$1]=$1;next} {if(!($1 in a)) print $0}\' %s %s > %s'%(r"\t",remain_QNAME_file,intermediate_outFile,outSam)
    ret = getOneShellOutput(cmd)
    if len(ret[1])!=0:
...
bam_outFile = args.outFile
cmd = "samtools view -H %s | cat - %s | samtools sort -O bam -o %s"%(args.bam,outSam,bam_outFile)
ret = getOneShellOutput(cmd)
print(ret[1])
if not isfile(bam_outFile):
    raise Exception("Making final bam file.\nERROR: %s"%(ret[1]))
print(time.strftime("%c"))
getOneShellOutput("rm %s"%(outSam))
