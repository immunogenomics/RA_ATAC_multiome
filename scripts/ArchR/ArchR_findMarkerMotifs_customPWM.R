...
parser <- ArgumentParser(description='Add custom motifs to ArchR project')
...
parser$add_argument('motif_file', metavar='motif_file', help='motif file of PFMatrixList object')
...
if(!all(file.exists(c(project_dir,motif_file)))) stop("Input file(s) don't exist.")
...
print_time("Load motif list")
motif_list <- readRDS(motif_file)
motif_list
if(!is(motif_list,'PWMatrixList')) stop('motif_file must be a PWMatrixList object')
...
proj <- addMotifAnnotations(ArchRProj = proj,
                            motifPWMs = motif_list,
                            annoName=paste(sep='',motif_prefix,"Motif"),
                            width=motif_width,cutOff=motif_pval,force=motif_force) 
