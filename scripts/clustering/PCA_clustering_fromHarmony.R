...
parser <- ArgumentParser(description='Harmony clustering of PCA matrix file')
parser$add_argument('pcaMat_file', metavar='pcaMat_file', help='feature x PC file')
...
parser$add_argument('this_nPCs', metavar='this_nPCs', type='integer', help='number of PCs to use')
parser$add_argument('--harmony_nclust', metavar='harmony_nclust', type='integer', help='number of soft kmeans clusters in Harmony ')
...
if(!all(file.exists(pcaMat_file,meta_file))) stop("Input file(s) don't exist.")
...
print_time('Load PCA mat file')
pcaMat <- readRDS(pcaMat_file)
if(this_nPCs>ncol(pcaMat)){
    stop(paste('Asked for',this_nPCs,'PCs; only have',ncol(pcaMat)))
} else if(this_nPCs<ncol(pcaMat)){
    cat(paste('Subsetting from',ncol(pcaMat),'PCs to the asked for',this_nPCs,'\n'))
    pcaMat <- pcaMat[,1:this_nPCs]
} #if equal, do nothing!
dim(pcaMat)
pcaMat[1:3,1:3]


print_time('Cluster Resolution Inputs')
...
use_meta <- FALSE
if(all(rownames(pcaMat) %in% rownames(meta))){
    meta <- meta[rownames(pcaMat),]
...
harmonyObj <- HarmonyMatrix(pcaMat, meta, harmony_col_toUse, 
                            nclust = harmony_nclust,sigma = harmony_sigma,
